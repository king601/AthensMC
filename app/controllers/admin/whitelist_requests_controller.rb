class Admin::WhitelistRequestsController < ApplicationController
  ...
  def charts
    ...
    @whitelist_requests_by_status = WhitelistRequest.all.group(:status).group_by_week(:created_at).count
    @last_month_chart = WhitelistRequest.all.group_by_day(:created_at).count
  end
